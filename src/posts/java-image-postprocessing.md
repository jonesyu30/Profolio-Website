...
1. Compare each rectangle with the input image, and calculate the difference between the two images.
1. Select the first half with the least difference, and kill the rest.
1. For each of the remaining rectangles, generate a variance which is similar to the original rectangle, but with a slight difference.
1. Repeat step 2 to 4 until the difference is small, or the maximum number of generations is reached.
1. Go to step 1 and repeat the process, until a certain number of generations is reached.
